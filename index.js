...
let totalContributuions = GAMES_JSON.reduce((acc, game) => {
    return acc + game.backers;
}, 0);

// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = totalContributuions.toLocaleString('en-US');

// grab the amount raised card, then use reduce() to find the total amount raised
const raisedCard = document.getElementById("total-raised");

// use reduce() to count the total amount pledged by summing pledged
...
let display = `A total of ${raisedCard.innerHTML} has been raised for ${GAMES_JSON.length} \
games. Currently, ${numUnfunded} game${numUnfunded !== 1 ? 's' : ''} remain unfunded. \
